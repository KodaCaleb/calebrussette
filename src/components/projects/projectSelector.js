      ...
      <div className=" justify-center flex h-full m-1 w-1/2">
        {currentProject ? (
          ...
          <div className="self-center justify-center flex-row flex h-1/2 w-full">
            <div className="h-2/5 w-2/3 sm:flex text-start hidden text-themeWhite text-5xl font-thin flex-col justify-center">
                <TypeAnimation
            ...
            {projectData.map((data) => {
              return (
                <button
                  ...
                  key={data.id}
                  className="self-center sm:self-end text-end  opacity-80 hover:opacity-100 font-thin"
                >
                  {data.title}
                </button>
              );
